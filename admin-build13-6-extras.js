(function(){
  ...
  function enhanceWizard(){
    ...
    const title=$('#programWizardTitle');if(title&&!title.dataset.build){title.dataset.build='1361';const eye=wizard.querySelector('.eyebrow');if(eye)eye.textContent='BUILD 13.6.1 · Programme & Gruppen'}
  ...
  function packageCard(p){const status=p.status||(!p.active?'archived':p.publicVisible?'published':'draft'),label=status==='published'?'Veröffentlicht':status==='archived'?'Archiviert':'Entwurf',primary=status==='published'?'<button class="mini" data-package-action="unpublish">Depublizieren</button>':status==='draft'?'<button class="mini" data-package-action="publish">Veröffentlichen</button>':'<button class="mini" data-package-action="restore">Reaktivieren</button>',archive=status==='archived'?'':'<button class="mini" data-package-action="archive">Archivieren</button>';return `<article class="package-admin-card" data-package-id="${esc(p.id)}"><div><span class="status-chip ${status}">${label}</span><h3>${esc(p.nameDe)}</h3><p>${p.sessionsTotal} × ${p.sessionMinutes} Min. · ${new Intl.NumberFormat('de-AT',{style:'currency',currency:'EUR'}).format(p.priceCents/100)} · ${p.validityDays} Tage gültig</p><small>${status==='published'?'Auf der Angebotsseite sichtbar':status==='archived'?'Nicht mehr aktiv':'Gespeichert, aber nicht veröffentlicht'} · ${esc(p.id)}</small></div><div class="program-actions"><button class="mini edit" data-package-edit="${esc(p.id)}">Bearbeiten</button>${primary}${archive}<button class="mini danger" data-package-action="delete">Löschen</button></div></article>`}
