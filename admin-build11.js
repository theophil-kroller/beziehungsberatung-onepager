(function(){
  ...
  let activeDictation=null, dictationRecorder=null, dictationStream=null, dictationUpload=Promise.resolve(), dictationSequence=0, dictationTimer=null, dictationSeconds=0, dictationPoll=null, dictationAudioUrl='';
  ...
  function renderFlowDictation(d){
    const card=$('#flowDictationCard');if(!card)return;const hasText=!!(d?.editedTranscript||d?.originalTranscript),hasAudio=!!d?.audioAvailable;
    $('#flowDictationState').textContent=d?({recording:'Aufnahme läuft',saved:'Bereit zur Transkription',transcribing:'Whisper transkribiert lokal …',draft:'Transkriptentwurf bereit',error:'Audio gesichert · Transkription fehlgeschlagen',accepted:'In Sitzung gespeichert'}[d.status]||d.status):'Noch keine Aufnahme';
    $('#flowDictationStart').hidden=!!d||!navigator.mediaDevices?.getUserMedia;$('#flowDictationRetry').hidden=!hasAudio||d?.status==='transcribing'||d?.status==='accepted';$('#flowDictationPlay').hidden=!hasAudio;$('#flowDictationDiscard').hidden=!d||d?.status==='accepted';$('#flowDictationAccept').hidden=!hasText||d?.status==='accepted';$('#flowDictationTranscriptPanel').hidden=!hasText;
    if(hasText&&!$('#flowDictationTranscript').matches(':focus'))$('#flowDictationTranscript').value=d.editedTranscript||d.originalTranscript||'';
    if(d?.status==='transcribing'){clearTimeout(dictationPoll);dictationPoll=setTimeout(()=>reloadFlowDictation(),1200)}
    if(d?.status==='error')dictationMessage(d.error||'Die Aufnahme bleibt erhalten. Bitte erneut transkribieren.','error');else if(d?.status==='accepted')dictationMessage('Geprüft übernommen und in dieser Sitzung gespeichert. Die Rohnotizen bleiben unten sichtbar.','ok');else if(d?.status==='draft')dictationMessage('Bitte Transkript prüfen. Danach als Rohnotizen übernehmen und anschließend mit lokaler KI strukturieren.','ok');else if(d?.status==='saved')dictationMessage('Aufnahme lokal gesichert. Starte jetzt die Transkription.','ok');
  }
  ...
  async function openFlowDictationInPlace(booking){
    const card=$('#flowDictationCard');if(!card)return false;card.scrollIntoView({behavior:'smooth',block:'center'});if(!await ensureVaultInFlow())return false;const ref=booking.customerIdentityEmail||booking.email;
  ...
  async function startFlowDictation(){
    ...
    try{await ensureVaultInFlow();const x=await window.BDVault.post('/dictation/start',{origin:'session',ref:booking.customerIdentityEmail||booking.email,displayName:booking.name,targetSessionId:booking.eventId,sessionDate:String(booking.start||'').slice(0,10),durationMinutes:60,contextLabel:`Sitzungsnotiz · ${booking.name}`,language:'de'});activeDictation=x.dictation;dictationSequence=0;dictationUpload=Promise.resolve();dictationSeconds=0;dictationStream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true}});const mime=dictationMime();dictationRecorder=new MediaRecorder(dictationStream,mime?{mimeType:mime}:undefined);
      ...
      dictationRecorder.onstop=async()=>{clearDictationTimer();dictationStream?.getTracks().forEach(t=>t.stop());dictationStream=null;try{await dictationUpload;await window.BDVault.post('/dictation/finalize',{id:activeDictation.id,language:'de'});dictationMessage('Aufnahme gesichert. Whisper transkribiert jetzt lokal …','ok');await reloadFlowDictation()}catch(e){dictationMessage('Die Aufnahme bleibt lokal erhalten. '+e.message,'error')}};
      dictationRecorder.start(5000);dictationTimer=setInterval(()=>{dictationSeconds++;const el=$('#flowDictationTimer');if(el)el.textContent=`${Math.floor(dictationSeconds/60)}:${String(dictationSeconds%60).padStart(2,'0')}`},1000);$('#flowDictationStart').hidden=true;$('#flowDictationStop').hidden=false;$('#flowDictationPause').hidden=false;$('#flowDictationState').textContent='Aufnahme läuft';dictationMessage('Sprich frei. Jeder Abschnitt wird sofort verschlüsselt gespeichert.','ok');
  ...
  function stopFlowDictation(){if(dictationRecorder&&dictationRecorder.state!=='inactive')dictationRecorder.stop();$('#flowDictationStop').hidden=true;$('#flowDictationPause').hidden=true}
  async function acceptFlowDictation(){if(!activeDictation)return;const text=String($('#flowDictationTranscript').value||'').trim();if(text.length<2){dictationMessage('Bitte zuerst einen Text prüfen.','error');return}const raw=$('#flowRawNotes'),existing=String(raw?.value||'').trim();if(existing&&existing!==text&&!confirm('Die vorhandenen Rohnotizen durch das geprüfte Transkript ersetzen?'))return;try{await window.BDVault.post('/dictation/update',{id:activeDictation.id,editedTranscript:text});const x=await window.BDVault.post('/dictation/accept',{id:activeDictation.id,autoDeleteAudio:$('#flowDictationAutoDelete').checked});activeDictation=x.dictation;if(raw)raw.value=text;renderFlowDictation(activeDictation);$('#flowAiReviewNote').textContent='Transkript übernommen · jetzt „Mit lokaler KI strukturieren“ wählen, Entwurf prüfen und anschließend speichern.';$('#flowAiStructureBtn')?.focus()}catch(e){dictationMessage(e.message,'error')}}
  ...
  async function retryFlowDictation(){if(!activeDictation)return;try{await window.BDVault.post('/dictation/transcribe',{id:activeDictation.id,language:'de'});await reloadFlowDictation()}catch(e){dictationMessage(e.message,'error')}}
  ...
  function injectUi(){
    ...
    $('#flowAiPrepBtn').onclick=generatePrep;$('#flowAiRefresh').onclick=getAiStatus;$('#flowAiStructureBtn').onclick=structureNotes;$('#flowDictationStart').onclick=startFlowDictation;$('#flowDictationStop').onclick=stopFlowDictation;$('#flowDictationAccept').onclick=acceptFlowDictation;$('#flowDictationPlay').onclick=playFlowDictation;$('#flowDictationRetry').onclick=retryFlowDictation;$('#flowDictationDiscard').onclick=discardFlowDictation;$('#flowDictationPause').onclick=()=>{if(!dictationRecorder)return;if(dictationRecorder.state==='recording'){dictationRecorder.pause();clearDictationTimer();$('#flowDictationPause').textContent='Fortsetzen'}else if(dictationRecorder.state==='paused'){dictationRecorder.resume();$('#flowDictationPause').textContent='Pause';dictationTimer=setInterval(()=>{dictationSeconds++;const el=$('#flowDictationTimer');if(el)el.textContent=`${Math.floor(dictationSeconds/60)}:${String(dictationSeconds%60).padStart(2,'0')}`},1000)}};
    getAiStatus();
  }
