(function(){
  ...
  let activeDictation=null, dictationRecorder=null, dictationStream=null, dictationUpload=Promise.resolve(), dictationSequence=0, dictationTimer=null, dictationSeconds=0, dictationPoll=null, dictationAudioUrl='', draftSaveTimer=null;
  ...
  function renderFlowDictation(d){
    ...
    $('#flowDictationStart').hidden=!!d||!navigator.mediaDevices?.getUserMedia;$('#flowDictationTranscribe').hidden=!d||!hasAudio||!['saved','error'].includes(d.status);$('#flowDictationRetry').hidden=!hasAudio||d?.status==='transcribing'||d?.status==='accepted';$('#flowDictationPlay').hidden=!hasAudio;$('#flowDictationDiscard').hidden=!d||d?.status==='accepted';$('#flowDictationAccept').hidden=!hasText||d?.status==='accepted';$('#flowDictationTranscriptPanel').hidden=!hasText;
  ...
  async function transcribeFlowDictation(){if(!activeDictation)return;try{await window.BDVault.post('/dictation/transcribe',{id:activeDictation.id,language:'de'});await reloadFlowDictation()}catch(e){dictationMessage(e.message,'error')}}
  ...
  function injectUi(){
    ...
    $('#flowAiPrepBtn').onclick=generatePrep;$('#flowAiRefresh').onclick=getAiStatus;$('#flowAiStructureBtn').onclick=structureNotes;$('#flowDictationStart').onclick=startFlowDictation;$('#flowDictationStop').onclick=stopFlowDictation;$('#flowDictationTranscribe').onclick=transcribeFlowDictation;$('#flowDictationAccept').onclick=acceptFlowDictation;$('#flowDictationPlay').onclick=playFlowDictation;$('#flowDictationRetry').onclick=retryFlowDictation;$('#flowDictationDiscard').onclick=discardFlowDictation;$('#flowDictationPause').onclick=()=>{if(!dictationRecorder)return;if(dictationRecorder.state==='recording'){dictationRecorder.pause();clearDictationTimer();$('#flowDictationPause').textContent='Fortsetzen'}else if(dictationRecorder.state==='paused'){dictationRecorder.resume();$('#flowDictationPause').textContent='Pause';dictationTimer=setInterval(()=>{dictationSeconds++;const el=$('#flowDictationTimer');if(el)el.textContent=`${Math.floor(dictationSeconds/60)}:${String(dictationSeconds%60).padStart(2,'0')}`},1000)}};
    $('#flowDictationCard').hidden=true;
    $('#flowInlineMic').onclick=async()=>{const ok=await openFlowDictationInPlace(flowCurrent());if(ok){$('#flowDictationCard').hidden=false;await startFlowDictation()}};
