(function(){
  ...
  async function createAccessPairing(){
    ...
    try{
      ...
      try{const QR=await loadLocalQr();await QR.toCanvas(dlg.querySelector('#docPairQr'),url,{width:290,margin:2})}catch(error){dlg.querySelector('#docPairQr').hidden=true;dlg.querySelector('#docPairQrImg').hidden=true;const note=document.createElement('div');note.className='doc-warning';note.innerHTML='<strong>QR-Code konnte nicht erzeugt werden.</strong><br>Die lokale QR-Komponente wurde nicht geladen. Bitte einmal Strg+F5 ausführen. Der geheime Pairing-Link wird weiterhin nicht an externe Dienste gesendet.';dlg.querySelector('#docPairQrImg').after(note);console.error('BD local QR error',error)}
