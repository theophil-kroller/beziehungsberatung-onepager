(function(){
  ...
  function renderSessions(){
    ...
    const searchBlob=s=>[s.date,s.durationMinutes,s.dictatedNote,s.focus,s.dynamics,Array.isArray(s.interventions)?s.interventions.join(' '):s.interventions,s.response,s.agreements,s.nextFocus,s.privatePrep].map(v=>String(v||'')).join(' ').replace(/\s+/g,' ').trim().toLowerCase();
    const consultationSearch=[consultation?.date,consultation?.topic,consultation?.reason,consultation?.mainProblem,consultation?.goals,consultation?.agreements,consultation?.nextStep].map(v=>String(v||'')).join(' ').replace(/\s+/g,' ').trim().toLowerCase();
    const items=display.map(s=>`<button class="vault-session-nav-item" type="button" data-session-select="session:${esc(s.id)}" data-session-card="${esc(s.id)}" data-session-search="${esc(searchBlob(s))}"><span class="vault-session-nav-title">Sitzung ${numbered.get(String(s.id))}</span><span class="vault-session-nav-meta">${esc(fmtDate(s.date))}${s.durationMinutes?` · ${esc(s.durationMinutes)} Min.`:''}</span><span class="vault-session-nav-preview">${esc(sessionExcerpt(s).slice(0,180))}</span></button>`).join('');
    const searchableConsultation=consultationItem.replace('data-session-select="consultation"',`data-session-select="consultation" data-session-search="${esc(consultationSearch)}"`);
    wrap.innerHTML=`<div class="vault-session-split"><aside class="vault-session-index"><div class="vault-session-index-tools"><div class="vault-session-search"><span aria-hidden="true">⌕</span><input type="search" data-vault-session-search placeholder="In Sitzungen suchen …" aria-label="In allen Sitzungsinhalten suchen"><button type="button" data-vault-search-clear title="Suche leeren" aria-label="Suche leeren">×</button></div><button class="vault-session-add" type="button" data-vault-add-session title="Neue Sitzung dokumentieren" aria-label="Neue Sitzung dokumentieren">＋</button></div><div class="vault-session-search-meta" data-vault-session-search-meta>${display.length} Sitzungen</div><div class="vault-session-index-scroll">${items||'<div class="vault-session-index-empty">Noch keine Sitzungen dokumentiert.</div>'}${searchableConsultation}</div></aside><section class="vault-session-detail" data-session-detail></section></div>`;
    ...
    const search=wrap.querySelector('[data-vault-session-search]'),clear=wrap.querySelector('[data-vault-search-clear]'),meta=wrap.querySelector('[data-vault-session-search-meta]');
    const applySearch=()=>{const q=String(search?.value||'').trim().toLowerCase();let count=0;let first=null;wrap.querySelectorAll('[data-session-select]').forEach(btn=>{const hay=String(btn.dataset.sessionSearch||'');const visible=!q||hay.includes(q);btn.hidden=!visible;if(visible){count++;if(!first)first=btn}});if(meta)meta.textContent=q?`${count} Treffer${count===1?'':' · durchsucht werden alle Sitzungsfelder'}`:`${display.length} Sitzungen`;const active=wrap.querySelector('[data-session-select].active');if(active?.hidden&&first)first.click();};
    search?.addEventListener('input',applySearch);clear?.addEventListener('click',()=>{if(search){search.value='';search.focus();applySearch()}});
  }
