(function(){
  ...
  async function openFlowDictation(booking){
    ...
    if(!token){window.open(VAULT+'/offline','_blank','noopener');msg('Der lokale Vault wurde geöffnet. Bitte dort entsperren und das Diktat starten.','err');return}
    const popup=window.open('about:blank','_blank');
    try{const x=await vaultRequest('/dictation/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({origin:'session',ref:booking.customerIdentityEmail||booking.email,displayName:booking.name,targetSessionId:booking.eventId,sessionDate:String(booking.start).slice(0,10),durationMinutes:Math.max(0,Math.round((bookingEnd(booking)-new Date(booking.start).getTime())/60000)),contextLabel:`Sitzungsnotiz · ${dt(booking.start)}`,language:'de'})}),url=VAULT+'/offline?dictation='+encodeURIComponent(x.dictation.id);if(popup)popup.location.href=url;else window.open(url,'_blank','noopener')}catch(e){if(popup)popup.close();msg(e.message,'err')}
  }
