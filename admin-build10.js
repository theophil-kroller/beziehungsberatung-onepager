(function(){
  ...
  function openFinanceView(){
    ...
    $('#viewEyebrow').textContent='Buchhaltung';$('#viewTitle').textContent='Buchhaltung';$('#viewSubtitle').textContent='Einnahmen, offene Honorarnoten und Zahlungswege auf einen Blick.';$('#newInquiryBtn').style.display='none';renderFinance();
  }
  ...
  function ensureUi(){
    const defs=$('.svg-defs');
    if(defs&&!$('#i-flow'))defs.insertAdjacentHTML('beforeend','<symbol id="i-flow" viewBox="0 0 24 24"><path d="M4 5h4v4H4V5Zm0 10h4v4H4v-4Zm12-5h4v4h-4v-4ZM8 7h3a3 3 0 0 1 3 3v1m0 2v1a3 3 0 0 1-3 3H8"/></symbol><symbol id="i-finance" viewBox="0 0 24 24"><path d="M4 19V9m5 10V5m5 14v-7m5 7V3M3 21h18"/></symbol>');
    const bookingsNav=$('.nav-item[data-view="bookings"]');
    if(bookingsNav&&!$('[data-build10-flow-nav]'))bookingsNav.insertAdjacentHTML('afterend','<button class="nav-item flow-nav-special" data-view="flow" data-build10-flow-nav type="button"><svg><use href="#i-flow"/></svg><span>Session Flow</span><em class="flow-nav-badge" id="flowNavCount">0</em></button>');
    const invoicesNav=$('.nav-item[data-view="invoices"]');
    if(invoicesNav&&!$('[data-build10-finance-nav]'))invoicesNav.insertAdjacentHTML('afterend','<button class="nav-item" data-view="finance" data-build10-finance-nav type="button"><svg><use href="#i-finance"/></svg><span>Buchhaltung</span></button>');
    ...
    if(openCard){
      ...
      openCard.title='Buchhaltung öffnen';
    ...
    if(dashGrid&&!$('#flowDashList'))dashGrid.insertAdjacentHTML('afterend',`<section class="panel flow-dashboard-panel flow-dashboard-green"><div class="panel-head"><div><p class="eyebrow">Offene Sitzungsaufgaben</p><h2>Noch nicht abgeschlossen</h2><p class="muted">Nur Sitzungen, bei denen Nachbereitung oder Dokumentation noch offen ist.</p></div><button class="text-button" id="flowOpenView" type="button">Alle offenen Flows →</button></div><div id="flowDashList" class="flow-list"></div></section>`);
  ...
  function appendAttention(pending){const host=$('#attention');if(!host)return;host.querySelectorAll('[data-build10-flow-attention]').forEach(x=>x.remove());if(!pending.length)return;host.querySelectorAll('.empty').forEach(x=>x.remove());const parked=pending.filter(b=>derivedState(b,flowFor(b))==='parked').length;host.insertAdjacentHTML('afterbegin',`<div class="attention-item orange" data-build10-flow-attention><div><strong>${pending.length} Session-Dokumentation${pending.length===1?'':'en'} offen</strong><span>${parked?parked+' davon geparkt · ':''}Offene Sitzung fortsetzen und abschließen.</span></div><button class="mini flow-attention-link" id="flowAttentionOpen">Öffnen</button></div>`);$('#flowAttentionOpen').onclick=openFlowView;const badge=$('#attentionCount');if(badge){const base=[...host.children].filter(x=>!x.hasAttribute('data-build10-flow-attention')).length;badge.textContent=String(base+1)}}
  async function openFlow(eventId){current=(dashboard?.bookings||[]).find(b=>String(b.eventId)===String(eventId));if(!current)return;const f=flowFor(current);$('#flowDialogTitle').textContent=current.name;$('#flowDialogMeta').textContent=`${dt(current.start)} · ${current.typeLabel||'Termin'} · ${current.locationLabel||''}`;$('#flowCommercialPrompt').value=f?.commercialPrompt||'';['flowRawNotes','flowFocus','flowDynamics','flowInterventions','flowResponse','flowAgreements','flowNextFocus','flowPrivatePrep'].forEach(id=>{const el=$('#'+id);if(el)el.value=''});renderCloudContext();renderFlowSales();vaultData=null;loadVaultContext();const isPast=bookingEnd(current)<=now();setFlowTab(isPast?'post':'prep');$('#flowNextActions').classList.add('hidden');$('#flowMsg').textContent='';$('#flowDialog').showModal();document.dispatchEvent(new CustomEvent('bd:flow-opened',{detail:{booking:{...current}}}));if(isPast)await markPostStarted()}
  function renderCloudContext(){
    const email=norm(current.customerIdentityEmail||current.email),packages=(dashboard.packages||[]).filter(x=>norm(x.customerEmail)===email&&Number(x.remaining)>0),invoices=(dashboard.invoices||[]).filter(x=>norm(x.customerEmail)===email&&(x.status==='open'||x.status==='overdue')),past=(dashboard.bookings||[]).filter(x=>norm(x.customerIdentityEmail||x.email)===email&&x.status==='booked'&&new Date(x.start)<new Date(current.start)).sort((a,b)=>new Date(b.start)-new Date(a.start));
  ...
  function openClientSection(tab,productId=null){
    $('#flowDialog')?.close();
    const opener=[...document.querySelectorAll('[data-open-client]')].find(x=>norm(x.dataset.openClient)===norm(current.customerIdentityEmail||current.email));
  ...
  async function loadVaultContext(){const host=$('#flowVaultContext'),postStatus=$('#flowPostVaultStatus');try{const x=await vaultRequest('/client?ref='+encodeURIComponent(current.customerIdentityEmail||current.email));vaultData=x;const c=x.client||{},sessions=x.sessions||[],target=sessions.find(s=>String(s.id)===String(current.eventId))||null,draft=(x.sessionDrafts||[]).find(s=>String(s.id)===String(current.eventId))||null,preview=sessions.slice(0,3);const goals=(x.goals||[]).filter(g=>g.status!=='completed').slice(0,4);host.innerHTML=`<span class="flow-local-status ok">Vault verbunden</span><div class="flow-kv" style="margin-top:10px"><dt>Beratungsziel</dt><dd>${esc(c.overview||'Noch nicht hinterlegt')}</dd><dt>Aktueller Fokus</dt><dd>${esc(c.currentFocus||'Noch nicht hinterlegt')}</dd></div>${goals.length?`<div class="flow-goals">${goals.map(g=>`<span class="flow-goal">${esc(g.title||g.text||'Ziel')}</span>`).join('')}</div>`:''}${preview.length?`<div class="flow-last-sessions">${preview.map(s=>`<div class="flow-last-session"><strong>${esc(String(s.date||'').slice(0,10))} · ${esc(s.focus||'Sitzung')}</strong><span>${esc(s.nextFocus||s.agreements||s.response||'')}</span></div>`).join('')}</div>`:'<p class="muted micro">Noch keine Sitzungsdokumentation.</p>'}`;postStatus.innerHTML='<span class="flow-local-status ok">✓ Vault entsperrt · Notizen werden ausschließlich lokal gespeichert.</span>';prefillPostFromVault(target||preview[0],draft)}catch(e){host.innerHTML=`<span class="flow-local-status err">${esc(e.message)}</span><p class="muted micro">Für den Verlauf muss der lokale Vault entsperrt sein.</p><button class="mini edit" id="flowOpenVault" type="button">Dokumentation öffnen & Vault entsperren →</button>`;setTimeout(()=>$('#flowOpenVault')?.addEventListener('click',()=>openClientSection('documentation')),0);postStatus.innerHTML=`<span class="flow-local-status err">${esc(e.message)} – Notizen können noch nicht lokal gespeichert werden.</span>`}}
  function prefillPostFromVault(last,draft){const source=draft||last;if(!source)return;const map={flowRawNotes:source.dictatedNote,flowFocus:source.focus,flowDynamics:source.dynamics,flowInterventions:(source.interventions||[]).join(', '),flowResponse:source.response,flowAgreements:source.agreements,flowNextFocus:source.nextFocus,flowPrivatePrep:source.privatePrep};Object.entries(map).forEach(([id,value])=>{const el=$('#'+id);if(el&&String(value||'').trim())el.value=value});$('#flowNextFocus').placeholder=source.nextFocus?`Letzter nächster Fokus: ${source.nextFocus}`:'';$('#flowPrivatePrep').placeholder=source.privatePrep?`Letzte Vorbereitung: ${source.privatePrep}`:''}
  ...
  function flowPayload(extra={}){const f=flowFor(current);return {bookingEventId:current.eventId,customerId:current.customerId||null,customerEmail:current.customerIdentityEmail||current.email,customerName:current.name,sessionStart:current.start,sessionEnd:current.end,commercialPrompt:$('#flowCommercialPrompt').value.trim(),state:f?.state||'planned',...extra}}
  ...
  async function openForClient(email){
    ...
    const key=norm(email),all=(dashboard?.bookings||[]).filter(b=>b.status==='booked'&&norm(b.customerIdentityEmail||b.email)===key).sort((a,b)=>new Date(a.start)-new Date(b.start));
    if(!all.length){alert('Für diese Klientin bzw. diesen Klienten ist noch kein Termin vorhanden. Öffne zuerst die Terminverwaltung.');return false}
    const pending=all.filter(b=>bookingEnd(b)<=now()&&derivedState(b,flowFor(b))!=='completed').sort((a,b)=>new Date(b.start)-new Date(a.start));
    const upcoming=all.filter(b=>new Date(b.start).getTime()>=now());
    const target=pending[0]||upcoming[0]||all[all.length-1];
    if(target)openFlow(target.eventId);
    return !!target
  }
  document.addEventListener('bd:open-client-flow',e=>openForClient(e.detail?.email));
